...
tar -xzf "$tmp/$archive" -C "$tmp"; binary="$tmp/$root/scrollshift"; [ -f "$binary" ] && [ ! -L "$binary" ] || { echo 'malformed archive' >&2; exit 1; }; chmod 0755 "$binary"; mv -f "$binary" "$output"; echo "Downloaded ScrollShift $v to $output"
